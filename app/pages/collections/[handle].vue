...
// Helpers
const { getCollectionSortValues, getFilterValues } = useShopifyHelpers();
...
<template>
  <section v-if="collection" class="flex flex-col px-6 mb-20 py-10">
    <FilterMenu
      v-if="allProducts"
      :products="allProducts"
    />
    <div class="flex flex-wrap flex-col">
      <div class="col-start-1 flex flex-col mb-20">
        <h1 class="normal-case text-7xl font-semibold mb-5 tracking-tight leading-none">
          {{ collection.title }}
        </h1>
        <p class="text-[18px]">{{ collection.description }}</p>
      </div>
      <div class="hidden lg:flex mb-10">
      ...
      <div class="w-full flex justify-between items-center mb-5">
        <button
          class="flex items-center justify-center p-2 text-xl border-b-2 border-meivita-green text-meivita-green"
          @click="toggleFilter"
        >
          Filter & Sort
        </button>

        <span class="text-xl text-gray-500">{{ numberOfProducts }} Items</span>
      </div>
    </div>
    <div
      v-if="filteredProducts && filteredProducts.length"
      class="grid grid-cols-2 auto-rows-fr gap-x-6 gap-y-8 w-full mb-8 lg:grid-cols-5 lg:gap-y-12"
    >
