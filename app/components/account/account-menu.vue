...
<template>
  <div v-if="customer" class="col-span-1 size-full px-4 lg:border-r lg:border-zinc-200">
    <div class="flex flex-col gap-4 mt-6 lg:gap-10">
      ...
      <nav class="hidden lg:flex lg:flex-col lg:gap-1">
        <NuxtLink to="/account"
          ...
          :class="{ 'text-meivita-green-dark bg-zinc-100': route.path === '/account' }">
          <Icon name="ph:globe" class="h-5 w-5 shrink-0" />
          Dashboard
        </NuxtLink>
        <NuxtLink to="/account/orders"
          class="flex items-center px-2.5 py-2 gap-2.5 text-normalize rounded-md transition duration-200 hover:bg-zinc-100"
          :class="{ 'text-meivita-green-dark bg-zinc-100': route.path === '/account/orders' }">
          <Icon name="ph:tag" class="h-5 w-5 shrink-0" />
          Orders
        </NuxtLink>
        <NuxtLink to="/account/addresses"
          class="flex items-center px-2.5 py-2 gap-2.5 text-normalize rounded-md transition duration-200 hover:bg-zinc-100"
          :class="{ 'text-meivita-green-dark bg-zinc-100': route.path === '/account/addresses' || route.path.includes('/account/addresses/') }">
      ...
      <nav class="relative flex lg:hidden">
        <select id="navigation" v-model="selectedPage" name="SelectCountry"
          class="flex w-full py-2 px-4 normal-case bg-white border border-zinc-300 rounded-md appearance-none placeholder:text-meivita-green-dark focus:outline focus:outline-1 focus:outline-black"
          @change="navigateToSelectedPage">
          <option v-for="page in pages" :key="page.path" :value="page.path" :selected="currentPath === page.path">
            {{ page.label }}
          </option>
        </select>
