...
<template>
  <div v-if="colorOption" class="flex flex-col gap-2">
    ...
    <div class="flex flex-wrap gap-2">
      <NuxtLink v-for="color in sortedColors" :key="color.handle" :to="`/products/${color.handle}`"
        ...
        :class="[
          ...
          !color.isAvailable
            ...
            : 'text-meivita-green-dark'
        ]" :style="{ background: color.color }" :aria-label="`Color Option ${color.name}`"
        @mouseenter="updateColorName(color.name)" @mouseleave="resetColorName" />
    </div>
  </div>
</template>
