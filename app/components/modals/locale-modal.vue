...
<template>
  <Transition name="bg-fade" mode="out-in" appear>
    <section v-if="appStore.localeModalOpen" class="fixed inset-0 z-[200] bg-black/50" @click="closeModal" />
  </Transition>
  <Transition name="scale-in" mode="out-in" appear>
    <div v-if="appStore.localeModalOpen"
      ...
      <form class="flex flex-col my-6" @submit.prevent="updateLocalization">
        <div class="relative w-full mb-2.5">
          <select id="country" v-model="countryLocale" name="countryLocale"
            class="flex w-full py-2 px-3.5 normal-case bg-white border border-zinc-300 rounded-md appearance-none placeholder:text-meivita-green-dark focus:outline focus:outline-1 focus:outline-black">
      ...
      <button class="flex absolute top-2 right-2 ring-1 ring-transparent ring-offset-2 rounded-sm focus:ring-black"
        @click="closeModal">
