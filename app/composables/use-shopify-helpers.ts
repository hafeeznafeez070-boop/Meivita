...
export const useShopifyHelpers = () => {
  ...
  /**
 * Checks if a product is marked as a Best Seller.
 * @param product - The product object
 * @returns A boolean indicating if the product is a Best Seller
 */
  const isBestSeller = (product: ProductFragment): boolean => {
    return product.tags?.some(tag => tag.toLowerCase() === "best seller") ?? false;
  };

  /**
   * Checks if a product is marked as Top Rated.
   * @param product - The product object
   * @returns A boolean indicating if the product is Top Rated
   */
  const isTopRated = (product: ProductFragment): boolean => {
    return product.tags?.some(tag => tag.toLowerCase() === "top-rated") ?? false;
  };
